...
## 3. 静态环境规范 (`network/topology.yaml`)
...
- **接口优先的原则：** 我们在 `topology.yaml` 中确立了“接口的 `mode` 决定了 `link` 的合法性”这一原则。这使得我们的网络定义极其健壮，为未来支持更复杂的接口类型（如 `vlan`, `vxlan`）打下了坚实的基础。
...
- **目录结构:** `network/` 包含一个核心的 `topology.yaml` 文件和一个可选的 `mounts/` 目录。`mounts/` 用于存放所有待挂载的文件，其子目录以节点ID命名，实现了“约定优于配置”。
- **`topology.yaml`:**
    - **顶层字段:** `images`, `nodes`, `switches`, `links`。
