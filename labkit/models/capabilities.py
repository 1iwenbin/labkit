...
class Assertion(BaseLabbookModel):
    ...
    value: Optional[Any] = Field(
        None, description="Expected value (not needed for exists/not_exists)"
    )
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "assert": [{"path": "result.nexthop", "rule": "equals", "value": "10.0.1.2"}],
            }
        }
...
class Monitor(BaseCapability):
    ...
    max_samples: Optional[int] = Field(
        None, description="Maximum number of samples to collect during monitoring"
    )
    threshold: Optional[Dict[str, Any]] = Field(
        None, description="Threshold conditions for alerting (e.g., {'cpu_usage': '>80%'})"
    )
